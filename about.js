...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // تهيئة الخريطة
    function initMap() {
        const map = L.map('map').setView([29.0469, 31.0979], 12); // إحداثيات بني سويف
        ...
        // إضافة علامة للموقع
        L.marker([29.0469, 31.0979])
            .addTo(map)
            .bindPopup('Ehnasia City, Beni Suef<br>My Hometown')
            .openPopup();
    ...
    // تهيئة الخريطة عندما تكون الصفحة جاهزة
    ...
    document.querySelectorAll('.glass-card, .section-header, .timeline-item, .skill-progress').forEach(el => {
        el.style.opacity = '0';
        observer.observe(el);
    });
